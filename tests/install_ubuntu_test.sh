...
grep -q '^go 1.22$' "${ROOT_DIR}/go.mod"
grep -q 'GOTOOLCHAIN=local go env GOVERSION' "${ROOT_DIR}/scripts/install-ubuntu.sh"
grep -q 'GOTOOLCHAIN=local go test ./...' "${ROOT_DIR}/scripts/install-ubuntu.sh"
grep -q 'GOTOOLCHAIN=local go build' "${ROOT_DIR}/scripts/install-ubuntu.sh"
! grep -q 'GOTOOLCHAIN=auto' "${ROOT_DIR}/scripts/install-ubuntu.sh"
grep -q 'Go 1.22 or newer is required' "${ROOT_DIR}/scripts/install-ubuntu.sh"
